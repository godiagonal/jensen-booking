function modalPopup(align, top, width, padding, disableColor, disableOpacity, backgroundColor, borderColor, borderWeight, borderRadius, fadeOutTime, url, loadingImage, clickClose){
	...
	//lagt till
	$('#popup_close_tbl').remove();
	$('#outerModalPopupDiv').prepend('<table id="popup_close_tbl" style="position:absolute;" width="100%"><tr><td align="right"><a href="#" onclick="closePopup(300);" class="popup_close_button">X</a></td></tr></table>');
	...
	page_request.onreadystatechange=function(){
		if((url.search(/.jpg/i)==-1) && (url.search(/.jpeg/i)==-1) && (url.search(/.gif/i)==-1) && (url.search(/.png/i)==-1) && (url.search(/.bmp/i)==-1)) {
			...
			imageloader(url, containerid, loadingImage, width); //ändrat
		}
	}
...
//ändrat
function imageloader(url, containerid, loadingImage, width) {
	
	document.getElementById(containerid).innerHTML = '<div align="center"><img src="' + loadingImage + '" border="0" /></div>';
	document.getElementById(containerid).innerHTML='<div align="center"><img src="' + url + '" style="max-width:' + (width-20) + 'px;" border="0" /></div>';
